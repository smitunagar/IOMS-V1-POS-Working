...
export interface InventoryItem {
  ...
  quantityUsed: number; // Total quantity used since last restock
  totalUsed: number; // Total quantity used historically
...
const INVENTORY_ALERTS_KEY_BASE = 'inventoryAlerts';
...
function getUserAlertsStorageKey(userId?: string | null): string {
  return `${INVENTORY_ALERTS_KEY_BASE}_${userId || 'default'}`;
}
 ...
 */
export function getInventoryAlerts(userId: string | null = null): InventoryAlert[] {
  if (typeof window === 'undefined') {
    return [];
  }
  try {
    const alertsStorageKey = getUserAlertsStorageKey(userId);
    const storedAlerts = localStorage.getItem(alertsStorageKey);
    if (storedAlerts) {
      return JSON.parse(storedAlerts) as InventoryAlert[];
    }
    return [];
  } catch (error) {
    console.error(`Error accessing localStorage for alerts (user: ${userId}):`, error);
    return [];
  }
}

/**
 * Save inventory alerts for a user
 */
export function saveInventoryAlerts(alerts: InventoryAlert[], userId: string | null = null): void {
  if (typeof window === 'undefined') return;
  try {
    const alertsStorageKey = getUserAlertsStorageKey(userId);
    localStorage.setItem(alertsStorageKey, JSON.stringify(alerts));
  } catch (error) {
    console.error(`Error saving alerts to localStorage (user: ${userId}):`, error);
  }
}
 ...
 * Add a new alert
 */
export function addInventoryAlert(alert: Omit<InventoryAlert, 'id' | 'timestamp'>, userId: string | null = null): void {
  if (typeof window === 'undefined') return;
  
  const alerts = getInventoryAlerts(userId);
  ...
  saveInventoryAlerts(alerts, userId);
  
  // Trigger notification event
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent('inventory-alert', { detail: newAlert }));
  }
}
 ...
 */
export function acknowledgeAlert(alertId: string, userId: string | null = null): void {
  if (typeof window === 'undefined') return;
  ...
  saveInventoryAlerts(updatedAlerts, userId);
}

/**
 * Clear acknowledged alerts older than specified days
 */
export function clearOldAlerts(daysOld: number = 7, userId: string | null = null): void {
  if (typeof window === 'undefined') return;
  ...
  const filteredAlerts = alerts.filter(alert => {
    if (!alert.acknowledged) return true; // Keep unacknowledged alerts
    const alertDate = new Date(alert.timestamp);
    return alertDate > cutoffDate; // Keep recent acknowledged alerts
  });
  
  saveInventoryAlerts(filteredAlerts, userId);
}

/**
 * Check inventory for low stock and expiry alerts
 */
export function checkInventoryAlerts(userId: string | null = null): InventoryAlert[] {
  if (typeof window === 'undefined') return [];
  ...
  inventory.forEach(item => {
    ...
    // Check for low stock
      ...
      newAlerts.push({
        ...
        type: 'low_stock',
        severity: item.quantity <= item.lowStockThreshold / 2 ? 'critical' : 'warning',
        message: `Low stock alert: ${item.quantity} ${item.unit} of ${item.name} remaining (threshold: ${item.lowStockThreshold} ${item.unit})`,
        itemName: item.name,
        itemId: item.id,
        timestamp: new Date().toISOString(),
      });
      ...
      if (isValid(expiry)) {
        ...
        if (daysUntil < 0) {
          ...
          newAlerts.push({
            ...
            type: 'expiry',
            severity: 'critical',
            message: `${item.name} expired on ${format(expiry, "PP")}!`,
            itemName: item.name,
            itemId: item.id,
            timestamp: new Date().toISOString(),
          });
        } else if (daysUntil <= 3) {
          // Critical expiry (3 days or less)
          newAlerts.push({
            id: `alert_${item.id}_expiry_critical`,
            type: 'expiry',
            severity: 'critical',
            message: `${item.name} expires in ${daysUntil} day(s) on ${format(expiry, "PP")}!`,
            itemName: item.name,
            itemId: item.id,
            timestamp: new Date().toISOString(),
          });
        } else if (daysUntil <= 7) {
          // Warning expiry (7 days or less)
          newAlerts.push({
            id: `alert_${item.id}_expiry_warning`,
            type: 'expiry',
            severity: 'warning',
            message: `${item.name} expires in ${daysUntil} day(s) on ${format(expiry, "PP")}`,
            itemName: item.name,
...
/**
 * Update inventory alerts and return new alerts
 */
export function updateInventoryAlerts(userId: string | null = null): InventoryAlert[] {
  if (typeof window === 'undefined') return [];
  
  const newAlerts = checkInventoryAlerts(userId);
  const existingAlerts = getInventoryAlerts(userId);
  
  // Add new alerts
  newAlerts.forEach(alert => {
    const exists = existingAlerts.some(existing => 
      existing.itemId === alert.itemId && 
      existing.type === alert.type && 
      !existing.acknowledged
    );
    
    if (!exists) {
      addInventoryAlert(alert, userId);
    }
  });
