  ...
  try {
    ...
    if (error instanceof Error) {
      if (error.message.includes('API key') || error.message.includes('FAILED_PRECONDITION')) {
        throw new Error('Invalid or missing Gemini API key. Please check your .env.local file and ensure you have a valid API key.');
      }
    }
    
    throw new Error(`AI menu extraction failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
