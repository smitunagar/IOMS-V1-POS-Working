...
export default function InventoryPage() {
  ...
  // 🎓 NEW: Auto-hide "New to inventory" section
  ...
  // Helper functions for UI - Enhanced storage extraction
  const extractStorageInfo = (aiHint: string) => {
    ...
    // If no specific storage info, generate based on category
    if (!result.temperature && !result.tips) {
      // This will be populated by the smart suggestions system
      result.temperature = 'Smart storage recommendations available';
      result.tips = 'Click for detailed storage guidance';
    }
    
    return result;
  };

  // Enhanced storage info with category-based intelligence
  const getSmartStorageInfo = (item: InventoryItem) => {
    const category = item.category?.toLowerCase() || '';
    const name = item.name?.toLowerCase() || '';
    
    // Enhanced storage recommendations based on common food items
    if (category.includes('meat') || name.includes('chicken') || name.includes('beef') || name.includes('pork')) {
      return {
        temperature: '🧊 Cold Storage: 0-4°C (32-40°F)',
        tips: 'Store in refrigerator. Use within 1-2 days for fresh meat. Keep raw meat separate from other foods.',
        type: 'cold',
        icon: '🧊'
      };
    }
    
    if (category.includes('dairy') || name.includes('milk') || name.includes('cheese') || name.includes('yogurt')) {
      return {
    ...
    if (category.includes('frozen') || name.includes('ice cream') || name.includes('frozen')) {
      return {
        temperature: '❄️ Freezer Storage: -18°C (0°F) or below',
        tips: 'Keep frozen until ready to use. Avoid temperature fluctuations.',
        type: 'frozen',
        icon: '❄️'
      };
    }
    
    if (category.includes('produce') || category.includes('vegetables') || name.includes('tomato') || name.includes('lettuce')) {
      return {
        temperature: '🧊 Cool Storage: 7-10°C (45-50°F)',
        tips: 'Store in refrigerator crisper drawer. Keep away from ethylene-producing fruits.',
        type: 'cool',
        icon: '🧊'
      };
    }
    
    if (category.includes('pantry') || category.includes('spices') || name.includes('flour') || name.includes('rice')) {
      return {
        temperature: '🌡️ Room Temperature: 18-24°C (65-75°F)',
        tips: 'Store in cool, dry place away from direct sunlight. Keep containers sealed.',
        type: 'room',
        icon: '🌡️'
      };
    }
    
    if (category.includes('beverages') || name.includes('juice') || name.includes('soda')) {
      return {
        temperature: '🧊 Cold Storage: 2-8°C (36-46°F)',
        tips: 'Refrigerate after opening. Store unopened items in cool, dry place.',
        type: 'cold',
        icon: '🧊'
      };
    }
    
    // Default room temperature storage
    return {
      temperature: '🌡️ Room Temperature: 18-24°C (65-75°F)',
      tips: 'Store in cool, dry place. Check specific storage requirements for this item.',
      type: 'room',
      icon: '🌡️'
    };
  };
    ...
    <TooltipProvider>
      <>
        ...
        <Card className="shadow-2xl border-0 bg-gradient-to-br from-white via-blue-50/30 to-purple-50/30 backdrop-blur-sm transform transition-all duration-300 hover:shadow-3xl">
          ...
          {/* 🎓 USER-FRIENDLY GUIDANCE SECTION - Auto-hide after 5 seconds */}
          {showNewUserGuide && (
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button onClick={handleDownloadTemplate} variant="outline" disabled={!currentUser} className="shadow-md hover:shadow-lg transition-all duration-300">
                <Download className="mr-2 h-4 w-4" /> Download CSV Template
              </Button>
              <Button onClick={() => fileInputRef.current?.click()} variant="outline" disabled={!currentUser} className="shadow-md hover:shadow-lg transition-all duration-300">
              ...
              <input
                ...
                className="hidden"
              />
              <Button onClick={handleAddNewItem} disabled={!currentUser} className="shadow-md hover:shadow-lg transition-all duration-300">
          ...
          {/* Enhanced Voice Control Panel - Clean and Elegant */}
          <div className="mt-4 p-6 bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 rounded-xl border-2 border-blue-200 shadow-lg backdrop-blur-sm">
            <div className="flex flex-wrap gap-3 items-center justify-between">
              <div className="flex flex-wrap gap-3">
                <Button
                  ...
                  size="sm"
                  onClick={isListening ? stopVoiceSearch : startVoiceSearch}
                  disabled={!voiceSupported || isProcessingVoice}
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent className="max-w-sm p-4" side="right">
                          <div className="space-y-3">
                            ...
                            {(() => {
                              const smartStorage = getSmartStorageInfo(item);
                              const originalStorage = extractStorageInfo(item.aiHint || '');
                              ...
                              return (
                                <>
                                  ...
                                  {originalStorage.recommendations && originalStorage.recommendations.length > 0 && (
                                    <div className="text-xs bg-gradient-to-r from-purple-50 to-indigo-50 p-3 rounded-lg border border-purple-200">
                                      <span className="font-medium text-purple-900 flex items-center mb-2">
                                        <span className="text-lg mr-2">📋</span>
                                        Additional Recommendations
                                      </span>
                                      <ul className="list-disc list-inside ml-2 mt-1 text-purple-800">
                                        {originalStorage.recommendations.map((rec, idx) => (
                                          <li key={idx}>{rec}</li>
                                        ))}
                                      </ul>
                                    </div>
                                  ...
                                  {originalStorage.allergens && originalStorage.allergens.length > 0 && (
                                    <div className="text-xs bg-gradient-to-r from-red-50 to-pink-50 p-3 rounded-lg border border-red-200">
                                      <span className="font-medium text-red-900 flex items-center mb-2">
                                        <span className="text-lg mr-2">⚠️</span>
                                        Allergen Information
                                      </span>
                                      <div className="text-red-800">{originalStorage.allergens.join(', ')}</div>
                                    </div>
                                  )}
      ...
      {/* 🔔 Unified Notification System - Enhanced 3D Design */}
      <div className="fixed top-20 right-4 z-50">
        ...
        <div className="relative">
          <Button
            ...
            variant="outline"
            size="sm"
            className="relative bg-gradient-to-br from-white via-blue-50 to-purple-50 border-2 border-blue-300/50 hover:border-blue-400 transition-all duration-300 shadow-xl hover:shadow-2xl transform hover:scale-110 hover:-translate-y-1 backdrop-blur-sm"
          >
            ...
            {hasNewSuggestion && (
              <>
                ...
                <div className="absolute -top-1 -right-1 h-3 w-3 bg-gradient-to-r from-red-500 to-pink-500 rounded-full shadow-lg" />
              </>
            )}
            <span className="ml-1 text-xs hidden sm:inline font-medium bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Quick Access</span>
          </Button>
          
          {/* Unified Dropdown with Essential Options Only */}
          {showUnifiedNotifications && (
            <div className="absolute top-12 right-0 w-80 max-w-[90vw] bg-white/95 backdrop-blur-sm rounded-xl shadow-2xl border-2 border-blue-200 p-6 animate-in slide-in-from-top-2 transform transition-all duration-300">
              {/* Header */}
              ...
              <div className="space-y-4">
                {/* Chat with SAM */}
                <div className="p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg border-2 border-blue-200 shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-[1.02]">
                  <Button
                    onClick={() => {
                      setShowChatbot(true);
                      setShowUnifiedNotifications(false);
                    ...
                    variant="ghost"
                    className="w-full justify-start text-left p-3 h-auto hover:bg-white/50 rounded-lg"
                  >
                    <div className="flex items-center">
                      <div className="text-2xl mr-3">🤖</div>
                      <div>
                        <div className="font-bold text-blue-900">Chat with Smart SAM</div>
                        <div className="text-sm text-blue-600">Get intelligent IOMS guidance and assistance</div>
                      </div>
                    </div>
                  </Button>
                </div>
                ...
                {/* Upload CSV */}
                <div className="p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border-2 border-green-200 shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-[1.02]">
                  <Button
                    onClick={() => {
                  ...
                  >
                    <div className="flex items-center">
                      <div className="text-2xl mr-3">📄</div>
                      <div>
                        <div className="font-bold text-green-900">Upload CSV for Menu/Inventory</div>
                        <div className="text-sm text-green-600">Smart conversion with AI optimization</div>
                      </div>
                    </div>
                  </Button>
                </div>
      ...
      {/* 🤖 Enhanced 3D Floating Smart SAM Chatbot */}
      <div className="fixed bottom-6 right-6 z-50">
        {/* Enhanced 3D Floating Chat Button */}
        <Button
          onClick={() => setShowChatbot(!showChatbot)}
          className="rounded-full h-16 w-16 bg-gradient-to-br from-blue-600 via-purple-600 to-indigo-700 hover:from-blue-700 hover:via-purple-700 hover:to-indigo-800 shadow-2xl hover:shadow-3xl transition-all duration-500 transform hover:scale-110 hover:-translate-y-2 animate-pulse border-2 border-white/20"
        >
          <MessageCircle className="h-7 w-7 text-white drop-shadow-lg" />
          <div className="absolute inset-0 rounded-full bg-gradient-to-br from-white/20 to-transparent"></div>
        </Button>

        {/* Enhanced 3D Chat Interface */}
        {showChatbot && (
          <div className="absolute bottom-20 right-0 w-96 h-96 bg-gradient-to-br from-white via-blue-50/30 to-purple-50/30 backdrop-blur-md rounded-2xl shadow-2xl border-2 border-white/20 flex flex-col animate-in slide-in-from-bottom-4 transform transition-all duration-500">
            {/* Enhanced Chat Header */}
            <div className="flex items-center justify-between p-4 border-b border-white/30 bg-gradient-to-r from-blue-50/70 to-purple-50/70 rounded-t-2xl backdrop-blur-sm">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 bg-gradient-to-br from-blue-500 via-purple-500 to-indigo-600 rounded-full flex items-center justify-center shadow-lg transform transition-all duration-300 hover:scale-110">
