...
export default function AiOrderAgentPage() {
  ...
  const handleProcessTranscript = () => {
    if (!transcript.trim()) {
      toast({ title: "No Transcript", description: "Please enter a transcript to process.", variant: "destructive" });
      return;
    }
    ...
    startProcessingTranscript(async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          toast({ title: "Processing Failed", description: result.error || "Unable to extract order.", variant: "destructive" });
        }
      } catch (error) {
  ...
  const handleConfirmAndCreateOrder = () => {
    if (!aiExtractedOrder) {
      toast({ title: "No Order", description: "Please process a transcript first.", variant: "destructive" });
      return;
    }
    ...
    startCreatingOrder(async () => {
      try {
        ...
        if (result.success && result.data) {
          toast({ title: "Order Created", description: `Order #${result.data.orderNumber} created successfully!` });
          ...
          toast({ title: "Order Failed", description: result.error || "Failed to create order.", variant: "destructive" });
        }
      } catch (error) {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Review AI-extracted items and add manual entries if needed.</CardDescription>
              </CardHeader>
              {!aiExtractedOrder ? (
                <CardContent>
                      ...
                      <div className="flex gap-2 items-end">
                            ...
                            <SelectContent>
                              {menuDishes.map(dish => (
                                <SelectItem key={dish.id} value={dish.id}>
                                  {dish.name} (${(typeof dish.price === 'number' && !isNaN(dish.price) ? dish.price : parseFloat(dish.price?.toString?.() ?? '') || 0).toFixed(2)})
                                </SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
                        ...
                        {isClient && menuDishes.length === 0 && (
                          <div className="px-4 py-2 text-muted-foreground text-sm">
                            No dishes available. Add dishes via the Ingredient Tool and refresh.
                          </div>
                        )}
                        <Input
                          type="number"
                          ...
                          <div key={idx} className="flex justify-between items-center p-2 border rounded-md">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <Badge variant={item.isMatched ? "default" : "secondary"}>
                                ...
                                {item.isMatched ? "Matched" : "No Match"}
                              </Badge>
                              <Button
                                variant="destructive"
                                size="sm"
            ...
            <Card className="bg-white/90 backdrop-blur-sm border-0 shadow-xl">
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between">
                    <span className="text-gray-600">Matched Items:</span>
                    <span className="font-semibold">
                      {processedOrderItems.filter(item => item.isMatched).length}
                    </span>
                  </div>
                  <div className="flex justify-between">
